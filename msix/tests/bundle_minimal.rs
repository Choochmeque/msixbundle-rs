...
fn make_msix(manifest: &str) -> (tempfile::TempDir, tempfile::NamedTempFile) {
    ...
    std::fs::write(src.path().join("AppxManifest.xml"), manifest).unwrap();
...
fn bundle_two_archs() {
    ...
    let bundle_out = tempfile::NamedTempFile::new().unwrap();
    ...
    bundle(&packages, bundle_out.path(), &identity).unwrap();

    // Read the bundle back.
    let f = std::fs::File::open(bundle_out.path()).unwrap();
    ...
    zip.by_name("AppxMetadata/AppxBundleManifest.xml").unwrap().read_to_string(&mut m).unwrap();
    ...
    let expected_x64_bytes = std::fs::read(x64_msix.path()).unwrap();
    let actual_bundle_bytes = std::fs::read(bundle_out.path()).unwrap();
...
fn parse_attr(manifest: &str, tag: &str, attr: &str) -> u64 {
    ...
    row[v_start..v_end].parse().expect("attr not a u64")
}
